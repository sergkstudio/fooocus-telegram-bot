...
class FooocusClient:
    ...
    def _initialize_api(self):
        """Инициализация необходимых параметров API"""
        self.client.predict(fn_index=35)
        self.client.predict(fn_index=34)
        self.client.predict("Inpaint or Outpaint (default)", fn_index=56)
        self.client.predict("Inpaint or Outpaint (default)", fn_index=57)
        self.client.predict(None, fn_index=32)
...
# Ожидание завершения генерации
            ...
            result = self.client.predict(fn_index=68)
            
            # Обработка результатов
            if isinstance(result, list) and len(result) >= 4:
                return self._parse_result(result[3])
            
        except Exception as e:
            logging.error(f"Generation error: {str(e)}")
            return None

    def _parse_result(self, gallery_data):
        """Парсинг данных галереи"""
        try:
            if isinstance(gallery_data, str):
                gallery_data = json.loads(gallery_data)
            
            if isinstance(gallery_data, list) and len(gallery_data) > 0:
                return gallery_data[0].get('url', '') if isinstance(gallery_data[0], dict) else gallery_data[0]
            
        except json.JSONDecodeError:
            return gallery_data[0][0] if isinstance(gallery_data, list) else ''
...
async def handle_message(update: Update, context):
    try:
        ...
        image_url = fooocus_client.generate_image(prompt)
